...
- [ ] In your own words, describe `actions`, `reducers` and the `store` and their role in Redux. What does each piece do? Why is the store known as a 'single source of truth' in a redux application?
    ...
    The Store is what redux uses to contain all of state throughout the application, the state is immutable but replaceable.
    Actions are the changes that are trying to be made to the store, but have to be handled by the reducers
    The Reducers take the actions, and decide what to do with them, and how to go about doing it.

- [ ] What is the difference between Application state and Component state? When would be a good time to use one over the other?

    Application state makes it so you dont have to pass state through each component down the tree by creating a single object, the store, to handle and interface the rest of the applications state. Its a better option to use Application state when your app is going to be big enough to not want to pass things down to each component.



- [ ] Describe `redux-thunk`, what does it allow us to do? How does it change our `action-creators`?

    Redux-thunk is a middleware that lets you handle asychronous operations inside of the action-creators by intercepting the flow and doing something before the actions get to the reducer.
